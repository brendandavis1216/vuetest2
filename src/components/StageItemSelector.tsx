...
import { ScrollArea } from '@/components/ui/scroll-area'; // Import ScrollArea for better content management
    ...
    <Card className="h-full flex flex-col">
      ...
      <CardContent className="flex-grow flex flex-col p-0">
        <Tabs value={activeCategory} onValueChange={setActiveCategory} className="flex flex-col flex-grow">
          <TabsList className="flex flex-wrap justify-start gap-1 p-2 border-b">
          ...
          <ScrollArea className="flex-grow p-4"> {/* Use ScrollArea here */}
            {categories.map(category => (
              <TabsContent key={category.name} value={category.name} className="mt-0">
                <div className="grid grid-cols-1 gap-3"> {/* Simplified grid for items */}
